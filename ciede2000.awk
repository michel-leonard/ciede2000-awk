...
BEGIN {
	...
	# Determines whether or not "--canonical" is passed on the command line.
	canonical = ARGV[1] == "--canonical" && !(ARGV[1] = "")
}

{
	...
	printf("%s,%.16g\n", $0, ciede2000($1, $2, $3, $4, $5, $6, kl, kc, kh, canonical))
}
...
function ciede2000(l1, a1, b1, l2, a2, b2, kl, kc, kh, canonical) {
	...
	if (M_PI + 1E-14 < (h2 < h1 ? h1 - h2 : h2 - h1)) {
		h_delta += M_PI		
	...
	t = 1.0	- 0.17 * sin(h_mean + M_PI / 3.0) \
		+ 0.24 * sin(2.0 * h_mean + M_PI * 0.5) \
		+ 0.32 * sin(3.0 * h_mean + 8.0 * M_PI / 15.0) \
		- 0.20 * sin(4.0 * h_mean + 3.0 * M_PI / 20.0)
	n = c1 + c2
	# Hue.
